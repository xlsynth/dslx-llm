...
```dslx-snippet
fn naive_sum<N: u32, K: u32>(xs: uN[N][K]) -> uN[N] {
    for (x, acc): (uN[N], uN[N]) in xs {
        acc + x
    }(zero!<uN[N]>())
}
...
fn prop_csa_tree_u3_k3(a: u3, b: u3, c: u3) -> bool {
    ...
    combine_csa(sum, carry) == naive_sum(xs)
}
