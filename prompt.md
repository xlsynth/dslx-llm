...
**No Turbofish Syntax** DSLX does not currently support Rust-style turbofish syntax with `::<>`. For explicit parametric calls, write `foo<u32:4>(x)`, not `foo::<u32:4>(x)`. For parameterized struct literals, write `MyStruct<u32:4> { field: value }`, not `MyStruct::<u32:4> { field: value }`.
